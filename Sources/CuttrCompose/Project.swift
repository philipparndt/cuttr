...
public struct TimelineEntry: Sendable, Equatable {
	...
	public static func == (a: TimelineEntry, b: TimelineEntry) -> Bool {
		a.source == b.source && a.transition == b.transition && a.label == b.label
			&& a.trim == b.trim && a.overlays == b.overlays
	}
	...
	public init(list: [ClipReference], transition: Transition = .cut,
	            overlays: [Overlay] = []) {
		self.init(source: .list(list), transition: transition, overlays: overlays)
	}
	...
	public init(group: String, entries: [TimelineEntry], transition: Transition = .cut,
	            overlays: [Overlay] = []) {
		self.init(source: .group(group, entries), transition: transition, overlays: overlays)
	}
...
/// Where an overlay is written down.
///
/// An index into `overlays:` was enough while that was the only place one could
/// be. Now that an entry carries its own, the panel, the strip and the effect
/// renderers all need to say *which* overlay they mean, and "the third one"
/// no longer picks out a single thing. A path and an index does.
public enum OverlayOrigin: Sendable, Equatable, Hashable {
	/// The nth of the project's top-level `overlays:`.
	case project(Int)
	/// The nth overlay written inside the timeline entry at this path.
	case entry(path: [Int], index: Int)

	/// The top-level index, for the places that are only ever about that list —
	/// the overlay table, which lists it and nothing else.
