...
public final class PropertiesPanel: NSView {
	...
	private func keyframes(_ index: Int, _ overlay: Overlay) {
		...
		for (position, key) in overlay.keys.enumerated() {
			...
			for parameter in parameters {
				add(keyRow(parameter, of: key, at: position, in: index,
	...
	private func keyRow(_ parameter: Overlay.Parameter, of key: Overlay.Key,
	                    at position: Int, in index: Int, inherited: Double) -> NSView {
		...
		let box = text(TakeWriter.number(shown, places: 3), width: 66, placeholder: "—") {
			...
			self?.setKey(index, position, parameter, cleaned.isEmpty ? nil : Double(cleaned))
		}
		if stated == nil {
		...
		let button = small(stated == nil ? "set" : "inherit") { [weak self] in
			self?.setKey(index, position, parameter, stated == nil ? shown : nil)
		}
	...
	private func span(of index: Int) -> Double {
		let found = resolved?.overlays.first {
			$0.source == index && $0.appearance == selectedSpan
		}?.duration
		return max(0.5, found ?? 2)
	}

	private func editKeys(_ index: Int, _ change: (inout [Overlay.Key]) -> Void) {
	...
	private func setKey(_ index: Int, _ position: Int,
	                    _ parameter: Overlay.Parameter, _ value: Double?) {
		editKeys(index) { keys in
			guard position < keys.count else { return }
