...
@Suite @MainActor struct OverlayKeyPanelTests {
	...
	private func panel(_ start: Project) -> (PropertiesPanel, () -> Project) {
		...
		panel.onChange = { [weak panel] next in
			box.project = next
			panel?.reload(next, vocabulary: ComposeDocument.Vocabulary(), selection: .overlay(0))
		}
		panel.reload(start, vocabulary: ComposeDocument.Vocabulary(), selection: .overlay(0))
